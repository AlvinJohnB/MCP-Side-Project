# MCP Side Project - Group 2

# Technical Specifications Document

## 1. Title Page

- **Project Name**: Airline Booking System Prototype (Front-End)  
- **Version**: Prototype v1  
- **Date**: June 24, 2025  
- **Author(s)**: Bootcamp Group 2 - Alvin John Bregaña, Krisha Mae Fabonan, Maria Cherilyn Abe  

---

## 2. Table of Contents

1. [Introduction](#3-introduction)  
2. [Overall Description](#4-overall-description)  
3. [Entity Relation Diagram](#5-entity-relation-diagram)  
4. [Features](#6-features)  
5. [Functional Requirements](#7-functional-requirements)  
6. [External Interface Requirements](#8-external-interface-requirements)  

---

## 3. Introduction

- **Purpose**: To develop a prototype Airline Booking web-application that allows users to browse flights, book flights, and make payments.  
- **Scope**: The application will include user registration, product catalog, checkout process, and integration with a payment gateway. It will not include advanced features such as user reviews or order tracking.  
- **References**: None  

---

## 4. Overall Description

- **Product Perspective**: The application is a standalone web app for travel agencies to upsell flight promos online.  
- **Product Functions**:
  - User registration and login  
  - Product browsing and search  
  - Booking management  
  - Checkout process  
  - Payment processing  
- **Operating Environment**:
  - **Client**: Modern web browsers (Chrome, Firefox, Safari)  

---

## 5. Entity Relation Diagram

![ERD](ERD.png)  

---

## 6. Features

- **User Registration and Login**: Users can create accounts and log in.  
- **Product Catalog**: Users can browse flights on the homescreen.  
- **Checkout Process**: Users enter passenger information and review their booking.  
- **Payment Gateway Integration**: Users can make payments securely using a payment gateway.  

---

## 7. Functional Requirements

### Use Cases

- **Use Case 1**: User Registration  
  - **Title**: Register a new user  
  - **Description**: Users can create an account with an email and password.  
  - **Actors**: End User  
  - **Preconditions**: User is on the registration page.  
  - **Postconditions**: User account is created and user is logged in.  
  - **Main Flow**: User fills up the form → clicks "Register" → system creates account and redirects to login page.  
  - **Alternate Flows**: User enters invalid email → system shows error.  

### System Features

- **Feature 1**: User Registration and Login  
  - **Description**: Allow users to register and log in.  
  - **Priority**: High  
  - **Inputs**: Email, password  
  - **Processing**: Validate input, create user account  
  - **Outputs**: User is logged in  
  - **Error Handling**: Show error messages for invalid input  

---

## 8. External Interface Requirements

- **User Interfaces**:
  - Registration/Login page  
  - Home page  
  - Book page  
  - Enter Passenger Info page  
  - Payment page  

- **Supporting Information**:
  - User flow diagrams  
  - Wireframes  

- **Revision History**:  
  - **v1.0**: Initial version - June 24, 2025  

---

# Trello Board

![Trello Screenshot](TrelloScreenshot.PNG)  
Link: [Trello Board](https://trello.com/b/1sL6G5hR/side-project)  

---

# Visual Prototype

Link: [GitHub Hosted Prototype](https://alvinjohnb.github.io/MCP-Side-Project/)  

---

# Airline Booking System – Functional Prototype Documentation

## 1. Project Overview

The Airline Booking System prototype demonstrates the core functionality of an airline reservation system. It showcases user interactions, including flight search, flight selection, booking, and confirmation.

**Purpose of Prototype**:
- Validate user experience and flow  
- Demonstrate key system features without full backend  
- Serve as a reference for final system design and development  

---

## 2. Prototype Scope

The prototype focuses on essential user interactions and system processes:  

- **User-Facing Features**:
  - Flight Search → Search flights by origin, destination, and date; display available flights with time and price  
  - Flight Selection → Choose a flight from search results  
  - Booking Form → Enter passenger details (name, email, etc.)  
  - Booking Confirmation → Display confirmation details including flight and passenger information  

- **Admin Features**:
  - View available flights  
  - Manage bookings  

---

## 3. Data Requirements

The prototype uses simplified mock data:  

**Example Flight Record**:  
- Flight Number: PR123  
- Origin: Manila  
- Destination: Cebu  

---

## 4. Functional Requirements

| Feature | Description | Input | Output |
|---------|-------------|-------|--------|
| Search Flights | Find flights based on origin, destination, and date | From, To, Date | List of available flights |
| Select Flight | Choose a flight from search results | Flight ID | Flight selected for booking |
| Enter Passenger Details | Input passenger information | Name, Email | Data stored in booking record |
| Confirm Booking | Display booking confirmation | Booking details | Confirmation message with flight info |

---

## 5. Non-Functional Requirements

- **Usability**: Simple and intuitive user interface  
- **Performance**: Fast search using mock data  
- **Portability**: Prototype runs on web browsers  
- **Reliability**: Mock data simulates expected system behavior  

---

## 6. Tools Used

- **Frontend**: React.js, HTML, CSS  
- **Backend Simulation**: Node.js / Express  
- **Data Storage**: MongoDB  

---

## 7. Limitations

- Flight search by destination only is not yet functional  

---

## 8. Testing

- **Flight Search Test**: Verify search returns correct results for given input  
- **Flight Selection Test**: Verify selected flight data is passed to booking form  
- **Booking Submission Test**: Verify confirmation displays correct passenger and flight details  

---

## 9. Future Enhancements

- User login and profile management  
- Seat selection and dynamic pricing  
- Search flights according to clicked destination by user  

---

## 10. Mock Flight Search & Booking Workflow (Visual)

          User opens Homepage
                  │
                  ▼
      Enters flight search details
                  │
                  ▼
    System displays available flights
                  │
                  ▼
          User selects a flight
                  │
                  ▼
      User enters passenger details
                  │
                  ▼
           Booking is confirmed
                  │
                  ▼
Confirmation page shows booking summary


---

## 11. Hosted Link

Link: [Vercel Deployment](https://flight-booking-app-pearl.vercel.app/)  
